...
def validateArguments(args):
    assert Path(args.inputFile).is_file(), f"`{args.inputFile}` doesn't exist."
    if not args.force:
        assert Path(args.inputFile).suffix == ".top", "`{args.inputFile}` does not have the .top extension; use `-f` to force"
...
def main(args):
    ...
    assert fres[0][2][0] == 'N' and fres[1][2][0] == 'H' and fres[2][2][0] == 'C' and fres[-2][2][0] == 'C', "N-terminal should be N-H-C-...-C-O" # but the O isn't in cmap
